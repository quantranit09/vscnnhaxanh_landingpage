---
plan: 03-01-pricing-partners
phase: 3
wave: 1
depends_on: [02-03-whyus-howitworks]
requirements_addressed: [PRICE-01, PRICE-02, PRICE-03, PART-01, PART-02]
files_modified:
...
# Plan 03-01: Pricing & Partners Section

## Objective
Xây dựng Pricing table show bảng giá tham khảo để minh bạch thông tin, kèm theo logo strip của các đối tác/khách hàng B2B đã phục vụ để tăng trust.

## Content Design

### Bảng Giá (Pricing Table)
```
Dịch Vụ | Đơn Giá Tham Khảo | Đơn Vị
--- | --- | ---
Vệ sinh văn phòng định kỳ | Từ 12,000đ | / m² / tháng
Vệ sinh nhà xưởng | Từ 8,000đ | / m²
Vệ sinh sau xây dựng | Từ 15,000đ | / m²
Dọn dẹp nhà cửa tổng thể | Từ 1,200,000đ | / gói
Giặt thảm / ghế sofa | Từ 15,000đ | / m² hoặc cái
```
*Disclaimer:* "Lưu ý: Bảng giá trên chỉ mang tính chất tham khảo. Giá thực tế phụ thuộc vào tình trạng công trình, mức độ cần làm sạch và yêu cầu cụ thể. Chúng tôi luôn khảo sát miễn phí trước khi báo giá chính xác."

### Partners (Logo Strip)
Sử dụng các building/company icons từ Lucide để làm mock logos:
`Building, Factory, Landmark, Store, Warehouse, Hospital`

## Tasks

### Task 1: Create Pricing.tsx

<read_first>
- `src/components/ui/SectionTitle.tsx` — use SectionTitle
...
<acceptance_criteria>
- [ ] `Pricing.tsx` export component Pricing
- [ ] Table renders 5 services with pricing
- [ ] Disclaimer note with `Info` icon is present
- [ ] Partners strip shows 6 mock logos
</acceptance_criteria>

### Task 2: Add to page.tsx and index.ts
...
<action>
Export `Pricing` from `src/sections/index.ts`.
Add `<Pricing />` to `src/app/page.tsx` directly after `<HowItWorks />`.
</action>

<acceptance_criteria>
- [ ] `page.tsx` imports and renders `Pricing`
- [ ] Build succeeds
</acceptance_criteria>
